...
import { sendPush, resolvePushSettings } from '../notifier.js';
...
router.put('/config', authRequired, (req, res) => {
  ...
  persist();
  res.json({ ok: true, config: { enabled: p.enabled, channel: p.channel, token: p.token, chatId: p.chatId, webhook: p.webhook } });
});
