...
export function buildProductCommentPrompt({ tone, prompt } = {}) {
  ...
  return (
    ...
    `禁止任何攻击性表达：不许质问、嘲讽、阴阳怪气、贬低或数落发布者、商家和其他用户，` +
    `不许用反问追究“为什么没写清楚”，也不许出现“糊弄”“逗我呢”“骗谁呢”“这是卖啥”“累不累”“凭什么”、` +
    `“坑人”“割韭菜”“把人当傻子”“离谱”“垃圾”“笑死”等冲人或贬损说法。` +
...
const RUDE_COMMENT_PATTERNS = [
  ...
  // 批次 42：补齐更隐蔽的贬损/攻击变体，避免只替换一个词就绕过检查。
  // 「离谱」「智商税」单独出现也可能是针对商品的克制评价，因此只在带明确对象时拦截，
  // 避免把「赠品比主件还香，离谱」「有点智商税的意思，不过喜欢就买」这类正常口语误伤。
  /坑人|坑爹|坑货|割韭菜|笑死|呵呵|扯淡|忽悠|忽悠人|骗(?:人|钱)/u,
  /(?:这(?:东西|款|价格)|商家|卖家|品牌方).{0,10}(?:智商税|离谱|垃圾|离大谱)/u,
  /(?:发布者|作者|楼主|商家|卖家|品牌方|消费者).{0,12}(?:傻|蠢|骗|坑|忽悠|糊弄|韭菜|没脑子)/u,
  /(?:你|您|这人|他|她).{0,10}(?:怎么回事|搞什么|有毛病|有问题|别来|少来|闭嘴)/u
];
