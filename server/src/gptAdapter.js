...
const RUDE_COMMENT_PATTERNS = [
  ...
  // 批次 40：「什么意思」曾在此处被误伤「这价什么意思，比昨天还贵」这类针对商品的正常吐槽，
  // 正则无法区分对象，故移除（误伤代价 > 漏拦代价）。
  /凭什么|累不累|就给个编号|就甩个|搞笑(?:吗|呢)|开什么玩笑/u,
  // 嘲讽贬损变体（批次 40 补漏）。
  // 「这也能叫/这也叫 + …」式贬低：「这能叫」+「叫」之间无「什」字，因此 `这(?:也|能|也能)叫` 写法
  // 不会匹配「这叫什么神仙价格」这类正面短句（其结构是「这叫什么」），零误伤。
