...
- `ADMIN_TOKEN` 为固定强随机值（脚本自动生成；**仅用于「系统更新」等高危操作**，与 API_TOKEN 隔离，切勿混淆）
...
- **H2**：✅ 已修复（2026-08-07）。更新接口改用独立 `requireAdmin` 鉴权：永远需要鉴权（不受 `REQUIRE_AUTH=false` 影响），并引入独立 `ADMIN_TOKEN`（部署脚本已自动生成）。即便 `API_TOKEN` 泄露也无法触发 `git pull + 重启`。
